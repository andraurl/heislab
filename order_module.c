...
is_active_order_on_floor(int floor)
{
	for(int button_type = 0; button_type < 3 ; button_type++)
	{
		...
		if(get_order_matrix(floor, button_type) == 1)
		{
			return 1; 
		}
	}
...
{
	int stop_for_BUTTON_COMMAND = get_order_matrix(floor, BUTTON_COMMAND) ; 
	int stop_for_BUTTON_CALL_UP = (get_order_matrix(floor, BUTTON_CALL_UP) && (get_last_known_direction() == DIRN_UP || !is_active_order_below())); 
	int stop_for_BUTTON_CALL_DOWN = (get_order_matrix(floor, BUTTON_CALL_DOWN) && (get_last_known_direction() == DIRN_DOWN || !is_active_order_above())); 
